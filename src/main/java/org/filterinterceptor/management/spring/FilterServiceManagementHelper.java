...
public class FilterServiceManagementHelper
{
  ...
  /**
   * Call this destroy method in spring config file
   */
  public void resetJmx()
  {
    FilterServiceManagement.resetMBean(filterService, mBeanDomain);
  }
   ...
   * 
   * @param filterService the service filter to use
   */
  @Required
  ...
  /**
   * Set the domain name, can be used if several filter service are exposed in the same JVM
   * 
   * @param domain the domain name to use. Can be null, the default value is {@link FilterServiceManagement#MBEAN_DEFAULT_DOMAIN}
   */
  // @Required //optional
